...
const useCheckAnswer = (setIsPaused: React.Dispatch<React.SetStateAction<boolean>>, setSeconds: React.Dispatch<React.SetStateAction<number>>, fetchNextQuestion: () => void) => {
    const [answerSubmitCount, setAnswerSubmitCount] = useRecoilState(answerSubmitCountState);
    ...
    const checkAnswer = async (inputAnswer: string, questionIndex: number, seconds: number, isPass = false): Promise<void> => {
        ...
        setAnswers((prevAnswers) =>
            prevAnswers.filter((answer) => answer.questionId !== currentQuestion.questionId),
        );
